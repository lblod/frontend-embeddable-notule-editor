...
export async function startApp({
  ...
  const container: Element = ensureElement(element);
...
export function ensureElement(selectorOrElement: Element | string): Element {
...
export async function renderInShadowRoot(options: RenderEditorOptions) {
  const { element } = options;
  const container = ensureElement(element);

  const editorContainer = document.createElement('div');
  const style = document.createElement('style');
  style.innerHTML = `@scope {
    ${(await import('../app/styles/app.scss?inline')).default}
  :scope {
        --au-white:#ffffff;
        --au-gray-100:#f7f9fc;
        --au-gray-200:#e8ebee;
